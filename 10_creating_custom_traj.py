...
elbow_joint_right_ind = mj_jntname2qposid("knee_angle_r", model)
test = 0.1 * np.sin(np.linspace(0, 2 * np.pi, N_steps))
tes3 = qpos[:, elbow_joint_right_ind]
qpos[:, elbow_joint_right_ind] += 0.5 * np.sin(np.linspace(0, 20 * np.pi, N_steps)).reshape(-1, 1)
...
qvel = qvel[1:-1]
qvel[:, elbow_joint_right_ind] = (qpos[2:, elbow_joint_right_ind] - qpos[:-2, elbow_joint_right_ind]) / (2 * env.dt)
